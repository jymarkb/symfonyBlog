...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->call(FeaturedCommentsSeeder::class);
    ...
    private function seedFeaturedComments(Post $featured, User $author): void
    {
        $sarah = User::firstOrCreate(
            ['email' => 'sarah.r@example.dev'],
            ['handle' => '@sarahrcodes', 'display_name' => 'Sarah R.', 'password' => bcrypt('password')],
        );
        $alexei = User::firstOrCreate(
            ['email' => 'alexei.k@example.dev'],
            ['handle' => '@alexeik', 'display_name' => 'Alexei K.', 'password' => bcrypt('password')],
        );
        $priya = User::firstOrCreate(
            ['email' => 'priya.m@example.dev'],
            ['handle' => '@priyam', 'display_name' => 'Priya M.', 'password' => bcrypt('password')],
        );
        $tom = User::firstOrCreate(
            ['email' => 'tom.d@example.dev'],
            ['handle' => '@tomd', 'display_name' => 'Tom D.', 'password' => bcrypt('password')],
        );
        ...
        // Author replies and top-level comment (AUTHOR badge tests)
        Comment::create([
            'post_id'    => $featured->id,
            'user_id'    => $author->id,
            'parent_id'  => $c1->id,
            'body'       => "Glad the framing landed. The \"one-way door\" is something I stole from Jeff Bezos's reversible vs irreversible decisions memo — turns out it applies equally well to content serialization.",
        ...
        // Author's own top-level comment — exercises the AUTHOR badge on a non-reply
        Comment::create([
            'post_id'    => $featured->id,
            'user_id'    => $author->id,
        ...
        // One more reply to $c4 — tests multi-reply thread
        Comment::create([
            'post_id'    => $featured->id,
            'user_id'    => $sarah->id,
            'parent_id'  => $c4->id,
